...
#               return i + 1
	...
	def get_pvr_frame(line):
		...
		TIME_DELTA = ((frame_time + offset) - pvr_time)
		TIME_DELTA1 = ((frame_time - frame_time1 + offset))
	...
	def play_video():
		...
		while (cap.isOpened()):
			...
			cv2.rectangle(frame, (x, y), (x + w, y + h), (0,0,0), -1)
    			#gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
			font = cv2.FONT_HERSHEY_SIMPLEX
			if frame_num - (fps*5) < frame_number:
...
w_1_btn_record_6_0 = Button(window, text="Record", command=record)
...
w_1_btn_exit_11_0 = Button(window, text="Exit", command=exit)
#text_box = tk.Text()
#entry = tk.Entry()
...
w_1_btn_record_6_0.grid(row=6, column=0, sticky="ew", padx=5)
...
w_1_btn_exit_11_0.grid(row=11, column=0, sticky="ew", padx=5)
