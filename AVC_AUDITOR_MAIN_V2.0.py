...
#		var.set(BANNER)
	...
	def tracker(line):
		...
		frame = 0
		output = 0
		def nothing(tracker_line): #(FRAME_NUMBER):
			i = 0
			global frame_num
			global pvr_line_number
			frame_num = Auditor.get_pvr_frame(tracker_line)[0] #(FRAME_NUMBER)
			banner_date,banner_time,banner_lane,banner_dir,banner_length,banner_speed,banner_class,banner_axle,banner_note = Auditor.banner_info(tracker_line)
			banner_speed = float(banner_speed)
			banner_speed = banner_speed*2.23694
			banner_speed = int(banner_speed)
			...
			if banner_lane == '2':
				i = 0
			pvr_line_number = tracker_line + count
			font = cv2.FONT_HERSHEY_SIMPLEX
			...
			line = pvr_line_number
			w_1_entry4_15_2.delete(0, END)
			w_1_entry4_15_2.insert(END, pvr_line_number)
			w_1_entry3_14_2.delete(0, END)
			w_1_entry3_14_2.insert(END, frame_num)
			cv2.imshow('frame', frame)
...
w_1_my_img = ImageTk.PhotoImage(Image.open('test_image.png'))
