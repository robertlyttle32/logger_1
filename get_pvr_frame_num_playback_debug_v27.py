...
#		var.set(BANNER)
	...
	def play_video():
		...
		while (cap.isOpened()):
			...
			if record_bkmark == True:
				out = cv2.VideoWriter(record_directory+banner_date+banner_time+EXT1,fourcc, 20.0, (1280,720))
				out.write(frame)
	...
	def tracker():
		...
		def nothing(FRAME_NUMBER):
			...
			cv2.imwrite('/media/bob/ssd128/Recordings/'+'pvr_'+banner_time+EXT1, frame)
			cv2.imshow('frame', frame)
